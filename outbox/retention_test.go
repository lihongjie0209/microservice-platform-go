...
type publishedStoreStub struct {
	counts []int64
	before []time.Time
}

func (s *publishedStoreStub) DeletePublishedBefore(_ context.Context, before time.Time, _ int) (int64, error) {
...
func TestRetentionCleanerRunOnceUsesBoundedBatches(t *testing.T) {
	t.Parallel()
	store := &publishedStoreStub{counts: []int64{2, 1}}
	cleaner, err := NewRetentionCleaner(store, RetentionConfig{Retention: 14 * 24 * time.Hour, BatchSize: 2})
	if err != nil {
		t.Fatal(err)
	}
	...
	if want := now.Add(-14 * 24 * time.Hour); !store.before[0].Equal(want) {
