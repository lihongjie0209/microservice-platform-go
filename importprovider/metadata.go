// Package importprovider defines the service-registry metadata contract used
// by services that implement platform.import.v1.ImportProviderService.
...
const (
	ProviderMetadataKey = "platform.import.provider"
	DatasetsMetadataKey = "platform.import.datasets"
)
...
func Metadata(datasets []Dataset) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode import datasets: %w", err)
	}
...
func Normalize(datasets []Dataset) ([]Dataset, error) {
	if len(datasets) == 0 {
		return nil, errors.New("at least one import dataset is required")
	}
	...
	for _, value := range datasets {
		...
		if !datasetCode.MatchString(value.Code) || value.Title == "" || value.MaxBatchSize < 1 || value.MaxBatchSize > 10000 {
			return nil, fmt.Errorf("dataset %q requires a valid code, title, and max batch size", value.Code)
		}
		if _, exists := seen[value.Code]; exists {
			return nil, fmt.Errorf("duplicate import dataset %q", value.Code)
		}
...
func ParseMetadata(metadata map[string]string) ([]Dataset, error) {
	if metadata[ProviderMetadataKey] != "true" {
		return nil, errors.New("instance is not an import provider")
	}
	var datasets []Dataset
	if err := json.Unmarshal([]byte(metadata[DatasetsMetadataKey]), &datasets); err != nil {
		return nil, fmt.Errorf("decode import datasets: %w", err)
	}
